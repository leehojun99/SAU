...
export default function Home({navigation}) {
  ...
  const [threadItems,setThreadItems] = useState([{"name":"김소현","major":"호텔 조리","title":"조리학","description":"조리학 책 구매 희망","isSell":true,"imageUrl":"https://~~!!"},
                       ...
                       {"name":"아바타","major":"경호","title":"채픔","description":"채플책 삽니다.","isSell":true,"imageUrl":"https://~~!!","price":"15000"},
      ...
      <View style={{flex: 1}}>
          ...
          />
            ...
            {threadItems.map((item, index) => (
              <ThreadItem onPress={() => {navigation.navigate('Order')}} name={item.name} major={item.major} isSale={!item.isSell} title={item.title} description={item.description} price={item.price}/>
            ))}
          </ScrollView>
